...
BEGIN
  busy <= NOT ss;  --high during transactions
  ...
  PROCESS(ss, clk)
  BEGIN
    IF(ss = '1' OR reset_n = '0') THEN                         --this slave is not selected or being reset
  ...
  PROCESS(ss, clk, tx_load_en, rx_req)
  ...
  BEGIN
    ...
    IF(ss = '1' OR reset_n = '0') THEN           --no transaction occuring or reset
      miso <= 'Z';
